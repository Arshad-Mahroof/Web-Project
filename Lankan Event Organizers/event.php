...
$priceFilter = isset($_GET['price_filter']) ? intval($_GET['price_filter']) : 0;

if ($priceFilter > 0) {
    $query = "SELECT * FROM package WHERE package_type='Event' AND price <= $priceFilter";
} else {
    $query = "SELECT * FROM package WHERE package_type='Event'";
}
...
<head>
    ...
    <title>View Event Packages</title>
