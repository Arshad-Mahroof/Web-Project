...
// Fetch Parties packages
$maxPrice = isset($_GET['max_price']) ? intval($_GET['max_price']) : null;

if ($maxPrice) {
    $query = "SELECT * FROM package WHERE package_type='Parties' AND price <= $maxPrice";
} else {
    $query = "SELECT * FROM package WHERE package_type='Parties'";
}
...
<head>
    ...
    <title>View Parties Packages</title>
