...
// Fetch Get Together packages
$maxPrice = isset($_GET['max_price']) ? intval($_GET['max_price']) : null;

if ($maxPrice) {
    $query = "SELECT * FROM package WHERE package_type='Get Together' AND price <= $maxPrice";
} else {
    $query = "SELECT * FROM package WHERE package_type='Get Together'";
}
...
<head>
    ...
    <title>View Get Together Packages</title>
