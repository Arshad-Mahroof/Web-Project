...
// Fetch DJ Parties packages
$maxPrice = isset($_GET['max_price']) ? intval($_GET['max_price']) : null;

if ($maxPrice) {
    $query = "SELECT * FROM package WHERE package_type='DJ Parties' AND price <= $maxPrice";
} else {
    $query = "SELECT * FROM package WHERE package_type='DJ Parties'";
}
...
<head>
    ...
    <title>View DJ Parties Packages</title>
